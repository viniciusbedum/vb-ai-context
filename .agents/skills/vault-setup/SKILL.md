...
2. If this looks like a fresh copy (the fictional examples are still present and no real user notes exist yet), briefly explain the two knowledge flows before asking anything: `X02 ✍️ My Thinking/` is for the user's own thinking, `X03 🗃️ Knowledge Base/` is for curated external sources, and `promote` is the only bridge between them. Keep this to a few sentences, not the full architecture doc.
